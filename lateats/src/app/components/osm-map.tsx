  ...
  if (loading) {
    ...
    return (
      ...
      >
        <MapContainer
          className="h-[88vh]"
          center={currPosition || [1.35, 103.82]}
          zoom={deniedPermission ? 12 : 30}
        >
          <TileLayer
          ...
          />
          {currPosition && (
            <Marker position={currPosition} icon={userIcon}>
                ...
                <Popup>
                  <a href={`/shop/${restaurant._id}`}>
                    ...
                    <div className="font-medium text-md text-center">
                      {restaurant.discount}% off till {restaurant.closingtime}
                    </div>
                  </a>
                </Popup>
              </Marker>
        ...
        <div className="bg-white px-4 shadow-inner shaodw-lg h-[12vh] flex justify-center p-4 lg:hidden">
          <button
            onClick={openMap}
            className="p-4 m-auto text-2xl font-semibold text-black bg-primary rounded-lg drop-shadow-md text-center w-full"
          >
            Home
          </button>
        </div>
